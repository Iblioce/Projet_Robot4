...
typedef enum { AVANCER, TOURNER, ARRETER, NB_ETATS} ETAT;
typedef enum { CAPT_ON, CAPT_OFF, DIST_FIN, NB_EVENTS} EVENT;
...
Transition tab_transition [NB_ETATS][NB_EVENTS]= {
  [AVANCER] = {
    ...
    [CAPT_ON] = {ARRETER, arreter},
    [DIST_FIN] = {ARRETER,  arreter}
  },
  [TOURNER] = {
    [CAPT_OFF] = {AVANCER, avancer},
    [CAPT_ON] = {ARRETER, arreter},
...
void ajuster_vitesse(){
  if(compteur_0 > compteur_1){
    TA1CCR1 = 1000;
    TA1CCR2 = TA1CCR1*((float)compteur_0/compteur_1);
  }else{
    TA1CCR2 = 1000;
    TA1CCR1 = TA1CCR2*((float)compteur_1/compteur_0);
  }
